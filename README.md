...
```python
model_clin.calculate_train_cindex(dataframe_timetoevent)
```
...
```python
model_clin.plot_selection_frequency()
```
...
```python
model_clin.define_2_risk_groups()
...
model_clin.define_2_risk_groups(plot=True)
```
...
```python 
#for risk scores
genomic_model.risk_scores_train

#for risk groups
clinical_model.get_train_risk_groups_2()

#or if you selected n=3 risk groups
clinical_model.get_train_risk_groups_3()
```
...
clinicogenomic_model.predict(dataframe_variables_test, classify=True, groups = 3) #or groups = 2 if you selected before 2 groups
```
